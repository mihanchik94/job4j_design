...
7.1. Объекты попали в один бакет или в разные?
7.2. Вызывался ли в этом случае equals() у объектов User или нет? Объяснить почему
При этом метод hashCode остается не переопределенным.

8.В классе User переопределить equals() и hashCode().
Добавить две пары  в карту. В качестве ключей использовать объекты из пункта 2, а в качестве значений new Object().
Вывести карту на печать. Описать полученный результат словами.
